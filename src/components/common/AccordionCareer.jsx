    ...
    <div className="space-y-1">
      {accData.items.map((item, index) => (
        <motion.div
          ...
          className={`border-b border-black/20 relative transition-all duration-500 ${openIndex === index ? "pl-5 2xl:pl-[40px]" : "pl-0"
            }`}
        >
          {/* LEFT ACTIVE BAR */}
          ...
          <button
            ...
            className={`w-full ${openIndex === index ? "pt-[20px] pb-3 xl:pb-[15px]" : "py-3 2xl:py-[32px]"
              } flex items-center justify-between text-left group transition-all duration-500`}
          >
            <div className="flex-1">
              <h3 className={`text-[20px] 2xl:text-29 leading-[1.3125]  text-paragraph  ${openIndex === index ? "font-bold" : "font-light 2xl:font-extralight"}`}>
                {item.title}
              </h3>
