           ...
           <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-y-6 lg:gap-y-10 xl:gap-y-[120px]">
            {
              data.items.map((item,index)=>( 
                <motion.div variants={moveUp(0.1*index)} initial="hidden" whileInView="show" viewport={{ amount: 0.2, once: true }} className="group border-l border-black/20 border-t-cmnbdr lg:border-t-transparent border-y lg:border-y-2 lg:border-y-transparent bdrrst hover:border-y-2 hover:border-y-[#30B6F9] 
                  ">
                  <h3 className="text-20 xl:text-24 2xl:text-29 font-light leading-[1.311] mb-4 lg:mb-[22px]  px-3 lg:px-10 pt-4 lg:pt-7">{item.location}</h3>
                 ...
                 <div className="p-4 lg:p-7 2xl:p-10  2xl:pb-[35px]">
                  ...
                  <ul className="flex flex-wrap gap-1 ulst pt-3 lg:pt-5 2xl:pt-[35px] mt-3 lg:mt-5 2xl:mt-[35px] border-t border-black/20">
                    { item.countriesList.map((country)=>(
                        <li className="text-16 2xl:text-19 text-paragraph font-light ">{country}<span className="text-[#30B6F9] pl-1 laststs">|</span> </li>
                      ))
                    }
                  </ul>
                 </div>
