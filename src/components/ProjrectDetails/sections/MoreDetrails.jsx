            ...
            <div className="flex  justify-between items-center border-b border-black/20">
              <div>
                ...
                {moredetrails.paragraphs.map((paragraph, index) => (
                  <motion.p key={index} variants={paragraphItem} initial="hidden" whileInView="show" viewport={{amount: 0.2, once: true}} className="text-19 font-light text-paragraph mb-3 xl:mb-5 last:lg:mb-[80px]">{paragraph}</motion.p>
                ))}
              </div>
            </div>
          </div>
