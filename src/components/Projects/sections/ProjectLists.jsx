...
const service = [
    { id: 1, title: "All" },
    { id: 2, title: "Engineering & Construction" },
    { id: 3, title: "MEP" },
    { id: 4, title: "Interior Fit-out" },
    { id: 5, title: "Facade" },
    { id: 6, title: "Facility Management" },
    { id: 7, title: "Water" },
    { id: 8, title: "Design Studio" },
];
...
const ProjectLists = () => {
    ...
    const filteredItems = useMemo(() => {
        ...
        if (selectedSector.id !== 1) {
            items = items.filter((item) => item.sector.toLowerCase() === selectedSector.title.toLowerCase());
        }

        if (selectedStatus.id !== 1) {
            // assuming your data has item.status
            items = items.filter((item) => item.status.toLowerCase() === selectedStatus.title.toLowerCase());
        }

        if (selectedCountry.id !== 1) {
            items = items.filter((item) => item.country.toLowerCase() === selectedCountry.title.toLowerCase());
        }

        if (selectedService.id !== 1) {
            // assuming your data has item.service
            items = items.filter((item) => item.service.toLowerCase() === selectedService.title.toLowerCase());
        }
