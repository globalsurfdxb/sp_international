
export const sustainabilityData = {
    mainSection: {
        title: "Building Responsibly, For Tomorrow",
        desc: ["Our leadership and expertise in various segments of our business portfolio comes from more than a century old dedication, experience, continuous innovation and investment in our people"],
        vdo: "./assets/videos/cmn-dummy-vdo.mp4",
        vdoPoster: "./assets/images/sustainability/vdo-poster.jpg"
    },
    expertiseData: {
        title: "Our Core Pillars of Sustainability",
        items: [
            {
                title: "Energy efficiency",
                image: "./assets/images/sustainability/core-pillars/img-2.jpg",
                desc: ["We make sure that all our employees attend the required Health & Safety trainings and are certified with the relevant certificates."],

            },
            {
                title: "Renewable Adoption",
                image: "./assets/images/sustainability/core-pillars/img-2.jpg",
                desc: ["We make sure that all our employees attend the required Health & Safety trainings and are certified with the relevant certificates."],

            },
            {
                title: "Waste Reduction",
                image: "./assets/images/sustainability/core-pillars/img-2.jpg",
                desc: ["We make sure that all our employees attend the required Health & Safety trainings and are certified with the relevant certificates."],

            },
            {
                title: "Green Buildings",
                image: "./assets/images/sustainability/core-pillars/img-2.jpg",
                desc: ["We make sure that all our employees attend the required Health & Safety trainings and are certified with the relevant certificates."],

            },

        ]
    },
    OngoingInvData: {
        title: "Ongoing Initiatives",
        items: [
            {
                image: "./assets/images/sustainability/ongoing/img-1.jpg",
                title: "Renewable Adoption",
                desc: "We make sure that all our employees attend the required Health & Safety trainings and are certified with the relevant certificates.",
            },
            {
                image: "./assets/images/sustainability/ongoing/img-2.jpg",
                title: "Renewable Adoption",
                desc: "We make sure that all our employees attend the required Health & Safety trainings and are certified with the relevant certificates.",
            },
            {
                image: "./assets/images/sustainability/ongoing/img-3.jpg",
                title: "Renewable Adoption",
                desc: "We make sure that all our employees attend the required Health & Safety trainings and are certified with the relevant certificates.",
            },
            {
                image: "./assets/images/sustainability/ongoing/img-4.jpg",
                title: "Renewable Adoption",
                desc: "We make sure that all our employees attend the required Health & Safety trainings and are certified with the relevant certificates.",
            },

        ]
    },
    certificationsData: {
        title: "Certifications & Standards",
        items: [
            {
                icon: "./assets/images/sustainability/certifications/c1.svg",
                title: "Certificate Name",
            },
            {
                icon: "./assets/images/sustainability/certifications/c2.svg",
                title: "Certificate Name",
            },
            {
                icon: "./assets/images/sustainability/certifications/c3.svg",
                title: "Certificate Name",
            },
            {
                icon: "./assets/images/sustainability/certifications/c1.svg",
                title: "Certificate Name",
            }
        ]
    },
    counterData : {
        svgSrc: "/assets/images/svg/svsv.svg",
        stats: [
            {
                value: "2.6 tCO₂",
                label: "Reduced emissions year",
            },
            {
                value: "500kL+",
                label: "Water Reused",
            },
            {
                value: "250+",
                label: "LEED Certified Projects",
            },
        ],
    },
    roadMapData: {
        title: "Future Roadmap",
        items: [
            {
                id: 1,
                number: "01",
                title: "Energy Efficiency",
                fullTitle: "Energy Efficiency",
                desc: "Lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor. Vivamus et eros nullam commodo blandit posuere dictumst. Phasellus gravida tellus nunc id luctus posuere pretium et posuere. Integer varius hendrerit urna nec faucibus aliquet ac sapien. In volutpat porttitor nisi in sodales. Sed ligula feugiat sed tellus dictum cursus lacus etiam at feugiat. Donec convallis felis quis urna fermentum ultrices eget pretium risus.",
                image: "./assets/images/sustainability/roadmap/carbon-reduction.jpg",
            },
            {
                id: 2,
                number: "02",
                title: "Green Buildings",
                fullTitle: "Green Buildings",
                desc: "Lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor. Vivamus et eros nullam commodo blandit posuere dictumst. Phasellus gravida tellus nunc id luctus posuere pretium et posuere. Integer varius hendrerit urna nec faucibus aliquet ac sapien. In volutpat porttitor nisi in sodales. Sed ligula feugiat sed tellus dictum cursus lacus etiam at feugiat. Donec convallis felis quis urna fermentum ultrices eget pretium risus.",
                image: "./assets/images/sustainability/roadmap/carbon-reduction.jpg",

            },
            {
                id: 3,
                number: "03",
                title: "Carbon Reduction",
                fullTitle: "Carbon Reduction & Renewable Integration",
                desc: "SPINT is committed to reducing its carbon footprint at every stage of construction and operations. By adopting renewable energy sources such as solar and other clean technologies, we aim to power project sites and facilities more sustainably.",
                image: "./assets/images/sustainability/roadmap/carbon-reduction.jpg",

            },
            {
                id: 4,
                number: "04",
                title: "Water & Waste Management",
                fullTitle: "Water & Waste Management",
                desc: "Lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor. Vivamus et eros nullam commodo blandit posuere dictumst. Phasellus gravida tellus nunc id luctus posuere pretium et posuere. Integer varius hendrerit urna nec faucibus aliquet ac sapien. In volutpat porttitor nisi in sodales. Sed ligula feugiat sed tellus dictum cursus lacus etiam at feugiat. Donec convallis felis quis urna fermentum ultrices eget pretium risus.",
                image: "./assets/images/sustainability/roadmap/carbon-reduction.jpg",

            }
        ]
    },
    featuredProjectsData: {
        title: "",
        items: [
            {
                image: "./assets/images/eng-constructions/featured-projects/prj1.jpg",
                title: "Oman Across Ages Museum",
                sector: "Social Infrastructure",
                sqft: "500000",
                location: "Oman"
            },
            {
                image: "./assets/images/eng-constructions/featured-projects/prj2.jpg",
                title: "Rove Downtown Hotel",
                sector: "Hospitality",
                sqft: "500000",
                location: "Dubai"
            },
            {
                image: "./assets/images/eng-constructions/featured-projects/prj3.jpg",
                title: "Altair",
                sector: "Hospitality",
                sqft: "500000",
                location: "Srilanka"
            },
            {
                image: "./assets/images/eng-constructions/featured-projects/prj2.jpg",
                title: "Oman Across Ages Museum",
                sector: "Hospitality",
                sqft: "500000",
                location: "Oman"
            },

        ]
    }

}