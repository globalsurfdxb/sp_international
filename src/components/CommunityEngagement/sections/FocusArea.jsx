...
const rthtRef = useRef(null);
  const [height, setHeight] = useState(0);

  useEffect(() => {
    function updateHeight() {
      if (rthtRef.current) {
        const h = rthtRef.current.getBoundingClientRect().height;
        setHeight(h);
      }
    }
    ...
    // If dynamic content: recompute on window resize
  ...
  return (
    <section className="  pt-12 pb-16 xl:pt-20 xl:pb-32 bg-f5f5 relative  overflow-hidden">
      ...
      <div
        ...
        className="max-h-[100dvh] overflow-y-auto  scrollbar-hidden   "
      > 
      <h2 className='text-60 font-light leading-[1.18] mb-5 lg:mb-8 xl:mb-25 invisible'>ss</h2>
        {focusData.areas.map((area, i) => (
          <div key={i} className=" mb-6 xl:mb-[63px] last:mb-0 last:xl:mb-0 h-[262px] md:h-[226px] xl:h-[262px]"
      >
      ...
      <div className='pl-6 lg:pl-10 2xl:pl-[107px]'>
         <h2 className='text-60 font-light leading-[1.18] mb-5 lg:mb-8 xl:mb-25'>{focusData.title}</h2>
        {focusData.areas.map((area, i) => (
          <div key={i} className="flex gap-3 pb-6 xl:pb-[63px] last:pb-0 last:xl:pb-0"
         
          > 
            <div className='flex flex-col lg:flex-row gap-6 lg:gap-12 2xl:gap-[181px] justify-between w-full'>
              <div>
              <div><p className='text-29 font-light mb-3 lg:mb-5 leading-[1.474]'>{area.title}</p></div>
              <div><p className='text-16 font-light leading-[1.75] text-paragraph max-w-[42ch]'>{area.description}</p></div>
            </div>
            <div  ref={rthtRef}>
              <img src={area.image} alt="" className='min-w-[258px] lg:min-w-[358px] xl:min-w-[478px]'/>
            </div>
            </div>
          </div>
        ))}
